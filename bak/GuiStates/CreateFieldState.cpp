...
CreateFieldState::CreateFieldState(Game &game) : game(game) {
    draw_offset = {10, 50};
    cell_size = {20, 20};

    field_width = game.getPlayer().getField().getWidth();
    field_height = game.getPlayer().getField().getHeight();
    ...
    if (event.type == sf::Event::KeyPressed) {
        switch (event.key.code) {
            ...
            case sf::Keyboard::Enter:
                game.createField(field_width, field_height);  // Create the field for the 1 player
                // player2.createField(field_width, field_height);  // Create the field for the 2 player
                next_state = GuiStates::CreateShips;  // Change to the next state
                break;
            case sf::Keyboard::Escape:
                game.createField(10, 10);
                next_state = GuiStates::Menu;  // Go back to the menu
                break;
            default:
                break;
        }
...
GuiStates CreateFieldState::changeState() {
    return next_state;
}
