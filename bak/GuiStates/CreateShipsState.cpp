...
CreateShipState::CreateShipState(Game& game) : game(game), active_box_index(0) {
    draw_offset = {10, 100};
    ...
    if (event.type == sf::Event::KeyPressed) {
        switch (event.key.code) {
            ...
            case sf::Keyboard::Enter: {
                createShips();
                next_state = GuiStates::PlacingShips;
            } break;
            case sf::Keyboard::Escape:
                // player.createShipManager(0, nullptr);
                game.createShips(0, nullptr);
...
GuiStates CreateShipState::changeState() {
    return next_state;
}
