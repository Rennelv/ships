...
CreateFieldState::CreateFieldState(const Game &game) : game(game) {
    draw_offset = {10, 50};
    cell_size = {20, 20};
    ...
    field_size_text.setString("CREATING FIELD");
}
...
void CreateFieldState::render(sf::RenderWindow &window) {
    window.clear();
    if (err) {
        field_size_text.setFillColor(sf::Color::Red);
...
void CreateFieldState::printErr(std::string msg) {
    field_size_text.setFillColor(sf::Color::Red);
    field_size_text.setString(msg);
    err = 1;
}